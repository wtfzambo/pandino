...
- Understand the existing code and the relevant external API before editing. Trace the real flow; never plan against imagined code. When uncertain external behavior blocks the current slice, use the cheapest authoritative source or direct probe that resolves it.
...
2. The main agent directly implements small, short fixes. Substantial implementation gets the approved bounded plan and the `implementer`; this routes writing cost only and does not determine reviewer routing. The implementer stops and reports when the plan contradicts the real code.
