...
4. Reviewer reports are evidence, not instructions. The orchestrator must adjudicate rather than relay them, checking each finding against the actual diff, the user request, documented contracts, and repository risk. Fix valid, in-scope, proportionate findings. Reject a finding only when it is demonstrably incorrect, duplicate, or out of scope, and state a checkable reason. Bundle valid product, scope, risk, cost, or proportionality trade-offs into one user checkpoint; valid spec scope-creep is a product decision, not an out-of-scope finding to dismiss, and must be surfaced in that checkpoint rather than silently kept or reverted.

Every must-fix from every reviewer needs an explicit disposition before commit. A valid finding that remains disputed, deferred, or intentionally unfixed requires explicit user acceptance before merge. Existing commit or merge authorization remains enough when reviews are clean or all findings were fixed or rejected on checkable evidence; do not seek ceremonial re-approval.

Standalone and whole-repository audits are report-only unless the user separately asks for implementation: the orchestrator synthesizes the worthwhile findings, flags anything disproportionate or decision-required, and waits before editing or creating tasks.
