...
Pandino also installs `fallback-runner`, a non-specialist, inspection-only escape hatch. Use it only when a specialist cannot launch or complete because its provider, quota, session, or pinned model is unavailable — never because a review found problems or the orchestrator dislikes its result. The orchestrator must supply an explicit alternate model, the failed specialist's canonical instructions verbatim, and the concrete task context; preserve the specialist's role and tool boundaries. For review work, choose a model different from the writer. Never invoke `fallback-runner` without an explicit model, which would silently inherit the parent, and visibly report every substitution.
