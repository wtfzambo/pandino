...
Final-reviewer verdict: merge, no must-fix. Accepted and fixed all three minors: documented unpublished local comparison URLs without adding ancestry/fork logic, made revision_json local, and added static parity evidence for the intentionally duplicated resolver helpers. Targeted taste/spec/test follow-ups passed. Rejected one test-review minor as demonstrably contrary to the declared contract: the test already states implementations must remain identical and intentionally ignores only comment-only changes, so byte-level code/format parity is the desired cheap drift guard. Final installer suite, Bash syntax, and diff hygiene pass.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Pandino installations now record the exact kit commit when determinable and install a read-only manual update checker. Remote installs pin the resolved archive, gracefully fall back to moving main with unknown provenance when lookup fails, and local installs use HEAD or unknown. README explains checker-first, merge-aware updates and the unpublished-local-link caveat without automatic session instructions. Verified through independent local/remote/fallback/failure/idempotence/immutability/helper-drift tests, real GitHub resolution and archive probes, taste/spec/test/docs/final reviews, syntax, and diff hygiene.
